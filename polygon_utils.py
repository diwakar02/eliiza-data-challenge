...
def may_intersect(multi_polygon_a, multi_polygon_b):
    multi_polygon_a = [[multi_polygon_a]]
    multi_polygon_b = [[multi_polygon_b]]
...
def may_intersect1(multi_polygon_a, multi_polygon_b):
    a_min_x, a_min_y, a_max_x, a_max_y = to_shape(multi_polygon_a).bounds
    b_min_x, b_min_y, b_max_x, b_max_y = to_shape(multi_polygon_b).bounds
...
def intersection_area_bool(multi_polygon_a, multi_polygon_b):
    if intersection_area(multi_polygon_a, multi_polygon_b) > 0.0:
        return True
    else:
        return False
